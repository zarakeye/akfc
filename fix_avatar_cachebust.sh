#!/bin/bash
# Fix : le nouvel avatar ne s affichait pas apres remplacement. Le publicId
# est FIXE (.../avatar) donc l URL ne change jamais + la route sert en
# immutable (cache 1 an) : le navigateur gardait l ancienne image. Ajout d un
# cache-buster (?t=timestamp) mis a jour a chaque upload/suppression.
# À lancer depuis la RACINE du monorepo : bash fix_avatar_cachebust.sh
set -euo pipefail
