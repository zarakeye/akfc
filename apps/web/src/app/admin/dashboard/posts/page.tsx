...
/**
 * Statut de publication d'un post (même logique que pour Event).
...
export default function PostsListPage(): JSX.Element {
  ...
  const fetchPostsAdmin = usePostStore((s) => s.fetchPostsAdmin);

  const [now] = useState(() => Date.now());

  useEffect(() => {
    void fetchPostsAdmin();
  }, [fetchPostsAdmin]);
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3 font-medium">
                      <Link
                        href={`/admin/dashboard/posts/${p.id}`}
