#!/usr/bin/env bash
###############################################################################
# ÉTAPE 2b — Composant front PersoPhotoUploader + page de montage testable
#
#   1. apps/web/src/features/admin/perso/PersoPhotoUploader.tsx (NOUVEAU)
#      Uploader perso léger : images seules, Cloudinary seul, destination
#      { kind: 'perso' }, barre de quota via storage.getPersoPhotoQuota,
#      blocage à remaining===0, conflits ignorés (jamais d'écrasement).
#   2. apps/web/src/app/(admin)/dashboard/mes-photos/page.tsx (NOUVEAU)
#      Point de montage minimal et testable (/dashboard/mes-photos). Le
#      placement final (dashboard + profil) viendra ensuite.
###############################################################################
set -euo pipefail
...
if ! grep -q "getPersoPhotoQuota" packages/backend/src/modules/storage/router.ts 2>/dev/null; then
  echo "ERREUR: 1b absent (getPersoPhotoQuota introuvable). Applique 1a/fix/1b/2a d'abord."; exit 1
fi

COMP="apps/web/src/features/admin/perso/PersoPhotoUploader.tsx"
if [ -f "$COMP" ]; then
  echo "Déjà appliqué (PersoPhotoUploader.tsx présent). Rien à faire."; exit 0
fi

mkdir -p apps/web/src/features/admin/perso
mkdir -p "apps/web/src/app/(admin)/dashboard/mes-photos"
...
FILE_EOF
echo "  [1] PersoPhotoUploader.tsx OK"

cat > "apps/web/src/app/(admin)/dashboard/mes-photos/page.tsx" << 'FILE_EOF'
'use client';
...
git add -A
git commit -m "feat(perso): PersoPhotoUploader (images, quota, block) + /dashboard/mes-photos mount"
echo "OK — 2b commité. Teste sur /dashboard/mes-photos."