#!/bin/bash
# Suppression de la page /actualites/[id] (le mur devient l'unique lieu de
# vie public d'un post) + repointage du lien admin vers l'ancre du mur.
# À lancer depuis la RACINE du monorepo : bash apply_suppression_actualites.sh
set -euo pipefail
[ -f pnpm-workspace.yaml ] || { echo "ERREUR : à lancer depuis la racine du monorepo." >&2; exit 1; }

echo "-> Suppression de apps/web/src/app/(public)/actualites"
rm -rf 'apps/web/src/app/(public)/actualites'

echo "-> apps/web/src/app/(admin)/dashboard/posts/[id]/edit/page.tsx"
echo "   lien de previsualisation -> ancre du mur"
cat > 'apps/web/src/app/(admin)/dashboard/posts/[id]/edit/page.tsx' << 'FILE_EOF'
"use client";
