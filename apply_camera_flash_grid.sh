#!/bin/bash
# Camera : ajout des boutons GRILLE (regle des tiers pour centrer) et FLASH.
# Flash : torch materiel si la capability existe (rare en frontale/desktop),
# sinon FLASH LOGICIEL (l ecran s illumine en blanc a la capture). Boutons en
# haut a droite, la grille est un overlay sur la preview.
# À lancer depuis la RACINE du monorepo : bash apply_camera_flash_grid.sh
set -euo pipefail
...
export function CameraCapture({
  ...
  // Toggle flash : torch matériel si dispo, sinon simple bascule d'état
  // (le flash logiciel se déclenchera à la capture).
  const toggleFlash = async () => {
    ...
    if (hasTorch) {
      ...
      try {
        ...
        // torch a échoué → on retombera sur le flash logiciel à la capture
        setHasTorch(false);
      }
    }
  };
  ...
  const handleCapture = () => {
    // Flash logiciel : si le flash est demandé mais qu'il n'y a pas de torch
    // matériel, on illumine l'écran en blanc un court instant AVANT de figer
    // l'image (le temps que la lumière éclaire le visage).
    if (flashOn && !hasTorch) {
      setScreenFlash(true);
      window.setTimeout(() => {
        doCapture();
