#!/bin/bash
# Fix fantome EN BASE : le deleteMany du prune supprime desormais la ligne
# Folder du dossier ET DE TOUS SES DESCENDANTS (comme la suppression
# Cloudinary est recursive). Sans ca, la ligne d'un sous-dossier survivait
# en base alors que Cloudinary etait propre → fantome affiche par le finder
# (qui lit la table Folder). Cause exacte du "fantome du premier dossier".
# À lancer depuis la RACINE du monorepo : bash fix_ghost_db.sh
set -euo pipefail
