...
export async function deleteCloudinaryFolderRecursive(
  ...
  console.log("[delRec] ENTER", folderPath);
  ...
  try {
    ...
    console.log("[delRec] subs", folderPath, JSON.stringify(subFolders.map((s) => s.path)));
  } catch (err) {
  ...
  try {
    ...
    console.log("[delRec] del OK", folderPath);
  } catch (err) {
    const desc = describeCloudinaryError(err);
    console.log("[delRec] del ERR", folderPath, JSON.stringify(desc));
    if (
      desc.message.toLowerCase().includes("not found") ||
      desc.http_code === 404
    ) {
...
FILE_EOF
echo "Trace posee. Refais le DnD fantome, colle [delRec] + [prune]."