#!/bin/bash
# Fix UX : le spinner s arretait a la fin de l upload, avant le chargement
# RESEAU de la nouvelle image -> trou visuel. Ajout d un etat `awaitingImage`
# maintenu de la validation jusqu au onLoad de la nouvelle <img> : le spinner
# reste affiche jusqu a l apparition effective du nouvel avatar.
# À lancer depuis la RACINE du monorepo : bash fix_avatar_spinner_until_loaded.sh
set -euo pipefail
...
export function AvatarUploader({
  ...
  const confirmPending = async () => {
    ...
    try {
      ...
      // On garde le spinner jusqu'au onLoad de la nouvelle image (mode
      // normal) : busy off, mais awaitingImage prend le relais.
            ...
            <div className="relative h-32 w-32 overflow-hidden rounded-full border border-gray-200 bg-gray-100">
              {currentUrl ? (
                ...
                <img
                  ...
                  alt="Votre avatar"
                  className="h-full w-full object-cover"
                  onLoad={() => setAwaitingImage(false)}
                  onError={() => setAwaitingImage(false)}
                />
              ) : (
                <div className="flex h-full w-full items-center justify-center text-gray-400">
                  <Camera className="h-8 w-8" />
