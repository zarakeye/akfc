#!/bin/bash
# Fix move : synchro MediaAsset (fullPath + status) dans l'adapter
# Cloudinary (elle n'existait pas) et status dans l'adapter R2 (trou
# identique). Cause racine du bug picker taolu.
# À lancer depuis la RACINE du monorepo : bash fix_move_sync.sh
set -euo pipefail
...
export function createCloudinaryStorageAdapter(
  ...
  return {
    ...
    async move(operation: StorageMoveOperation): Promise<void> {
      ...
      const srcDb = `${appRoot}/${operation.source.path}`;
      const dstDb = `${appRoot}/${operation.target.path}`;
      const topSegment = operation.target.path.split("/")[0];
      const nextStatus =
      ...
      if (operation.source.type === "file") {
        ...
        await prisma.$executeRaw`
          UPDATE "MediaAsset"
          SET "fullPath" = ${dstDb} || SUBSTRING("fullPath" FROM ${srcDb.length + 1}),
