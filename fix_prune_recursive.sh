#!/bin/bash
# Fix fantome changement de statut : le prune supprime le dossier
# Cloudinary RECURSIVEMENT (deleteCloudinaryFolderRecursive) — delete_folder
# simple echouait car le dossier gardait un sous-dossier vide non supprime.
# À lancer depuis la RACINE du monorepo : bash fix_prune_recursive.sh
set -euo pipefail
