#!/bin/bash
# Grille galerie : ombre portee au survol nettement renforcee (ombre
# personnalisee dense/large) pour un effet premier plan marque. Vue groupee :
# bordure autour de la discipline retiree.
# À lancer depuis la RACINE du monorepo : bash fix_gallery_shadow_border.sh
set -euo pipefail
...
# Typecheck vert (sinon set -e aurait interrompu) -> commit automatique.
echo
echo "Typecheck OK -> commit."
git add -A
git commit -m "feat(gallery): ombre survol renforcee + retrait bordure discipline"
