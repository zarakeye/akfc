#!/bin/bash
set -euo pipefail
[ -f pnpm-workspace.yaml ] || { echo racine >&2; exit 1; }
echo "-> packages/backend/src/modules/cloudinary/services/move.service.ts"
cat > 'packages/backend/src/modules/cloudinary/services/move.service.ts' << 'FILE_EOF'
...
async function renameAsset(
  ...
  // (vérifié en trace : le retour garde l'ancien dossier-entité). En
  // dynamic folders, le dossier-entité se met à jour via l'Admin API
  // `api.update(public_id, { asset_folder })` — un appel SÉPARÉ.
  try {
    const upd = await cloudinary.api.update(to, {
      asset_folder: nextAssetFolder,
      resource_type: resourceType,
      type: "authenticated",
    } as unknown as Parameters<typeof cloudinary.api.update>[1]);
    console.log(
      "[assetfolder] update OK — returned asset_folder=",
      (upd as Record<string, unknown>)?.asset_folder,
    );
  } catch (err) {
    console.log(
      "[assetfolder] update ERR",
...
echo "Si vert : refais un DnD, colle [rename] + [assetfolder] + verifie l API."