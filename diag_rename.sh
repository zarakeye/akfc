...
async function renameAsset(
  ...
  console.log("[rename] from=", from, "\n  to=", to, "\n  nextAssetFolder=", nextAssetFolder);
  try {
    const res = await cloudinary.uploader.rename(from, to, renameOptions);
    console.log("[rename] OK — returned asset_folder=", (res as Record<string, unknown>)?.asset_folder, "public_id=", (res as Record<string, unknown>)?.public_id);
  } catch (err) {
    console.log("[rename] ERR", (err as Error)?.message, JSON.stringify(err));
    throw err;
  }
...
FILE_EOF
echo "Trace posee. Refais un DnD, colle la sortie [rename]."