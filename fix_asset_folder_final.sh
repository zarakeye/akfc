#!/bin/bash
# FIX DEFINITIF dynamic folders : renameAsset deplace le public_id (rename)
# PUIS aligne le dossier-entite via api.update({ asset_folder }) — appel
# separe, car rename ignore asset_folder. Valide en test (dossier repare =
# delete_folder OK, asset_folder suit le public_id). Traces retirees.
# À lancer depuis la RACINE du monorepo : bash fix_asset_folder_final.sh
set -euo pipefail
