#!/bin/bash
# Fix placeholder avatar hors uploader (v2, corrige le typage : user est
# nullable dans SessionClient, garde !current.user ajoutee). Ajoute setAvatar
# au store de session, appele apres register/remove -> header a jour sans reload.
# À lancer depuis la RACINE du monorepo : bash fix_session_avatar_sync_v2.sh
set -euo pipefail
[ -f pnpm-workspace.yaml ] || { echo "ERREUR : racine du monorepo requise." >&2; exit 1; }

echo "-> apps/web/src/lib/stores/useSessionStore.ts"
cat > 'apps/web/src/lib/stores/useSessionStore.ts' << 'FILE_EOF'
