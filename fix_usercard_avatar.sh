#!/bin/bash
# Fix crash "Failed to parse src ... on next/image" : UserCard passait
# `user.avatar` (un publicId Cloudinary `AKFC/avatars/...`) directement a
# next/image, qui exige une URL absolue ou un chemin en `/`. On route
# desormais l avatar via le proxy by-public-id (comme partout ailleurs),
# fallback SVG conserve. C est ce qui plantait au clic dashboard (UserCard
# monte dans /dashboard/users).
# À lancer depuis la RACINE du monorepo : bash fix_usercard_avatar.sh
set -euo pipefail
[ -f pnpm-workspace.yaml ] || { echo "ERREUR : racine du monorepo requise." >&2; exit 1; }
echo "-> apps/web/src/features/admin/users/components/UserCard.tsx"
cat > 'apps/web/src/features/admin/users/components/UserCard.tsx' << 'FILE_EOF'
"use client";
