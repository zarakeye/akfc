#!/bin/bash
# Fix Cropper (v2) : apercu == resultat, rendu unifie fidele a la scene CSS.
# (v2 corrige la signature de drawCrop : parametre workspaceSize manquant.)
# À lancer depuis la RACINE du monorepo : bash fix_cropper_fidelity_v2.sh
set -euo pipefail
[ -f pnpm-workspace.yaml ] || { echo "ERREUR : racine du monorepo requise." >&2; exit 1; }
echo "-> apps/web/src/features/gallery-crop/components/Cropper.tsx"
cat > 'apps/web/src/features/gallery-crop/components/Cropper.tsx' << 'FILE_EOF'
"use client";
