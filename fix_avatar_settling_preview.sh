#!/bin/bash
# UX avatar : pendant que la nouvelle image charge via le RESEAU apres
# validation, on garde affiche SOUS le spinner la preview blob locale (image
# conservee, instantanee) au lieu d un fond vide. Une <img> cachee precharge
# l image reseau ; son onLoad libere la preview et bascule vers l avatar
# definitif. Transition continue, zero trou ni clignotement.
# À lancer depuis la RACINE du monorepo : bash fix_avatar_settling_preview.sh
set -euo pipefail
        ...
        <>
          ...
          >
            ...
            <div className="relative h-32 w-32 overflow-hidden rounded-full border border-gray-200 bg-gray-100">
              {/* Pendant la transition (awaitingImage) on affiche la preview
                  blob locale (settlingUrl) — instantanée, c'est l'image
                  conservée. Sinon l'avatar réseau (currentUrl). */}
              ...
              {/* Préchargement RÉSEAU de la nouvelle image, caché : son onLoad
                  libère la preview et coupe le spinner (bascule vers currentUrl). */}
