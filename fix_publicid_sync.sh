#!/bin/bash
# Fix synchro publicId au move : le publicId (qui porte le chemin complet et
# sert aux URLs de vignette) suit desormais fullPath+status. Sans ca, apres
# un deplacement, les apercus pointaient vers l'ancien emplacement -> 404 ->
# image generique. Corrige les DEUX blocs (fichier + dossier).
# À lancer depuis la RACINE du monorepo : bash fix_publicid_sync.sh
set -euo pipefail
