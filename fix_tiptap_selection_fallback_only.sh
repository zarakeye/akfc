#!/bin/bash
# Fix « selection Tiptap invisible » — VERSION SANS import SCSS global (le
# precedent essai important _variables.scss dans le layout a provoque un mur
# d erreurs, probablement du au prefixe underscore traite comme partial SCSS
# par Turbopack). Ici on ne garde QUE le FALLBACK sur --tt-selection-color
# dans paragraph-node.scss : la selection redevient visible (surlignage
# violet translucide) SANS dependre de l import des variables. Le layout est
# remis a son etat d origine (juste globals.css).
# À lancer depuis la RACINE : bash fix_tiptap_selection_fallback_only.sh
set -euo pipefail
...
git add -A
git commit -m "fix(editor): selection Tiptap visible via fallback (sans import scss global)"
