#!/bin/bash
# Camera, flash "on" repense : au lieu d un voile translucide (qui assombrit
# sans eclairer), le FOND de l overlay devient BLANC PUR pour illuminer au
# maximum. La preview video reste au centre comme une fenetre (le blanc
# l entoure, ne passe jamais devant). Barre d outils sur fond noir (lisible
# sur le blanc), bouton Capturer et bordure video adaptes au mode clair.
# À lancer depuis la RACINE du monorepo : bash apply_camera_flash_window.sh
set -euo pipefail
